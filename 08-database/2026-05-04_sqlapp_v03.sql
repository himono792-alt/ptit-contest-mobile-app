-- =========================================================
-- PTIT Contest Management System - PostgreSQL Schema v03
-- =========================================================
-- Thay đổi so với v02 (2026-05-04, cùng ngày):
--   + GAP-1: Thêm bảng contest_reviews (SV-11 đánh giá cuộc thi)
--   + GAP-2: Thêm bảng certificate_templates + issued_certificates
--            (BCN-06 xuất giấy chứng nhận có QR verify)
--   + GAP-3: Thêm bảng system_configs (AD-04 cấu hình hệ thống)
--            + seed 4 config mặc định
--   + Indexes mới cho 4 bảng trên
--   + Comments mới
--
-- Tổng số bảng: v02 = 39 → v03 = 43 (+4)
-- Coverage chức năng: v02 = 27/30 → v03 = 30/30 (full)
--
-- Renumber section: chèn "12) SYSTEM CONFIG" giữa Workflow và Audit.
-- Section 12 (Audit) → 13, 13 (Indexes) → 14, 14 (Comments) → 15.
--
-- Lưu ý sử dụng:
...
('certificate.qr_verify_url_base','http://localhost/verify/', 'STRING', 'Base URL ghép vào QR code chứng nhận', FALSE);
