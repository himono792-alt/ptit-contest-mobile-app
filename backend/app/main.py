...
_PROFILE_MIGRATION_STATEMENTS = [
    ...
    # Migrate 2026-06-11: qr_verify_url_base bám theo FRONTEND_BASE_URL (production
    # pages.dev đã ngừng — Docker local default http://localhost:8080). Idempotent:
    # chỉ UPDATE khi giá trị khác base hiện tại → tự sync mỗi lần đổi env.
    f"""UPDATE ptit_contest.system_configs
       SET config_value = '{settings.frontend_base_url}/verify/'
       WHERE config_key = 'certificate.qr_verify_url_base'
         AND config_value <> '{settings.frontend_base_url}/verify/'""",
]
...
app.add_middleware(
    ...
    # 2026-05-07: cho phép multi-level subdomain Cloudflare Pages preview
    # (preview deploy có dạng <hash>.<project>.pages.dev — cần (?:[a-z0-9-]+\.)+).
