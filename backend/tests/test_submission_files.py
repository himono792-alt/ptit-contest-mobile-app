...
async def _build_version(db, email=SV, slug="upload-flow-test"):
    ...
    contest = Contest(
        slug=slug, title=f"Contest {slug}",
        ...
        status=ContestStatus.ONGOING, created_by=user.user_id,
    )
    db.add(contest)
    await db.flush()
    rnd = ContestRound(
...
async def test_upload_rejects_non_owner(client, db):
    version_id = await _build_version(db, email="b22dccn003@ptit.edu.vn", slug="upload-flow-other")
