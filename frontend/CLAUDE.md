...
- Backend: Docker local (`http://localhost:8000` — `docker compose up -d` tại root repo). Production cloud đã ngừng 2026-06-11.
- Deploy web: container nginx trong compose (`http://localhost:8080`), build qua `docker compose build frontend`.
- Build flag: default `API_BASE=http://localhost:8000` (không cần flag khi chạy local); chỉ truyền `--dart-define=API_BASE=http://<IP-LAN>:8000` khi test LAN/APK.

## Design system rules — MUST follow

Tất cả UI mới hoặc sửa UI phải tuân thủ. Tham khảo `docs/DESIGN-REVIEW-CHECKLIST.md` cho full checklist.
...
- Reviewer paste link `docs/DESIGN-REVIEW-CHECKLIST.md` để check từng dimension.
...
Reference docs:
- `docs/audits/design-audit-2026-05-06.md` — audit gốc 7 dimension
- `tool/baseline-2026-05-06.txt` — baseline trước Phase A
- `tool/post-phase-a-2026-05-06.txt` — kết quả sau Phase A
- `docs/DESIGN-REVIEW-CHECKLIST.md` — checklist đầy đủ cho mọi PR
